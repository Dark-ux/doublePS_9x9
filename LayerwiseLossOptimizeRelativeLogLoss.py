...
def power_matrix_loss(P_current, P_target, p_floor=None, p_zero=None, p_leak=None, p_scale=None):
    if p_floor is None:
        p_floor = LOSS_P_FLOOR
    if p_zero is None:
        p_zero = LOSS_P_ZERO
    if p_leak is None:
        p_leak = LOSS_P_LEAK
    if p_scale is None:
        p_scale = LOSS_P_SCALE
    current = np.asarray(P_current, dtype=float)
    target = np.asarray(P_target, dtype=float)
    r = np.zeros_like(current, dtype=float)
    nonzero_mask = np.abs(target) >= float(p_zero)
    r[nonzero_mask] = np.abs(current[nonzero_mask] - target[nonzero_mask]) / (
        np.abs(target[nonzero_mask]) + float(p_floor)
    )
    # Theoretical-zero target entries tolerate small leakage; only leakage
    # above p_leak contributes to the relative-log loss.
    zero_mask = ~nonzero_mask
    r[zero_mask] = np.maximum(0.0, current[zero_mask] - float(p_leak)) / float(p_scale)
    return float(np.mean(r * np.log1p(r)))
...
def run_optimization(args):
    global LOSS_P_FLOOR, LOSS_P_ZERO, LOSS_P_LEAK, LOSS_P_SCALE
    LOSS_P_FLOOR = float(args.p_floor)
    LOSS_P_ZERO = float(args.p_zero)
    LOSS_P_LEAK = float(args.p_leak)
    LOSS_P_SCALE = float(args.p_scale)
    ...
    print(
        "Loss: relative_log_leak_tolerant, "
        f"p_floor={args.p_floor}, p_zero={args.p_zero}, "
        f"p_leak={args.p_leak}, p_scale={args.p_scale}"
    )
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        type=positive_float,
        default=1e-3,
        help="Floor used in the nonzero-target relative-log loss denominator: abs(target) + p_floor.",
    )
    parser.add_argument(
        "--p-zero",
        type=positive_float,
        default=1e-3,
        help="Target magnitude below this value is treated as theoretical zero.",
    )
    parser.add_argument(
        "--p-leak",
        type=positive_float,
        default=0.05,
        help="Allowed measured leakage for theoretical-zero target entries.",
    )
    parser.add_argument(
        "--p-scale",
        type=positive_float,
        default=0.05,
        help="Scale for leakage above p_leak in theoretical-zero target entries.",
    )
